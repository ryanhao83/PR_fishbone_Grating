...
a_nm    = 505.3        # lattice constant [nm]
h_spine = 0.530        # half-width of central spine [a]
W_rib   = 0.65        # rib length along x [a]
h_rib   = 0.56        # rib width along y [a]
...
def build_and_run(args):
    ...
    # ── Save ──────────────────────────────────────────────────────────
    save_path = os.path.join(output_dir, 'fdtd_71nm_data.npz')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda-min', type=float, default=1542.0)
    parser.add_argument('--lambda-max', type=float, default=1558.0)
    parser.add_argument('--snapshot-interval', type=float, default=20.0)
    parser.add_argument('--run-time', type=float, default=2600.0)
