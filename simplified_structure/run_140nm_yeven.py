...
def get_fields_at_k(params, k_val, resolution, target_bands):
    """Re-solve at a single k-point and extract E-field arrays for specific bands."""
    import meep as mp
    import meep.mpb as mpb
    import numpy as np
    ...
    # We need to run up to the maximum target band
    ...
    print(f"\nSolving modes at k={k_val} for field extraction (140nm)...")
    ms.run_yeven()
...
def plot_modal_fields(efields, freqs, k_val, a_nm, sy, sz, target_bands, out_path):
    """Plot |Ex|^2, |Ey|^2, |Ez|^2 cross-sections (y-z plane, x=mid) for each requested band."""
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    plt.suptitle(f"Mode Fields at k={k_val} (a={a_nm}nm, hsi=140nm)", fontsize=14)
...
def main():
    ...
    parser.add_argument('--num-bands', type=int, default=15)
    parser.add_argument('--save-plots', action='store_true')
    parser.add_argument('--save-data', action='store_true')
    parser.add_argument('--plot-modes', action='store_true', help='Plot mode fields at specific k points')
    parser.add_argument('--target-bands', type=int, nargs='+', default=[12, 13], help='Target bands to plot/save for mode fields')
    ...
    if args.plot_modes:
        ...
        for k_val in [0.486, 0.495]:
            ...
            plot_path = os.path.join(out_dir, f'yeven_140nm_modes_k{k_val:.3f}.png')
            plot_modal_fields(efields, freqs, k_val, BASE_PARAMS['a_nm'], sy, sz, args.target_bands, plot_path)
            
        if args.save_data:
            np.savez(os.path.join(out_dir, 'yeven_140nm_modes_data.npz'), **modes_data)
            print("Saved detailed mode data to yeven_140nm_modes_data.npz.")
